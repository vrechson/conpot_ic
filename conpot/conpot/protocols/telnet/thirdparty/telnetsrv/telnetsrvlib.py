...
# --------------------------- Environment Setup ----------------------------
    ...
    def setup(self):
        ...
        #    self.sendcommandset(self.DOACK[k], k)
        #for k in self.WILLACK.keys():
        #    self.sendcommandset(self.WILLACK[k], k)
...
# ------------------------- Telnet Options Engine --------------------------
    ...
    def sendcommandset(self, cmd, opt=None):
        "Send a telnet command (IAC)"
        print("chegou")
        if cmd in [DO, DONT]:
            if not (opt in self.DOOPTS):
